...
def _rounded_minute_offset(later: datetime, earlier: datetime) -> timedelta:
    return timedelta(minutes=round((later - earlier).total_seconds() / 60))
            ...
            timeframes.append(
                {
                    ...
                    "retrieved_at": iso_z(self.capture_time),
                    "bars": frame_bars,
                }
            )
        snapshot = {
            ...
            "quote": {"bid": 2000.0, "ask": 2000.2, "last": 2000.1, "spread_points": 20},
...
class MetaTrader5SnapshotAdapter(SnapshotAdapter):
    ...
    def capture(self, *, symbol: str, run_id: str, bars: int = 60, include_h4: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            # Some MT5 brokers encode server-local bar/tick time as Unix
            # seconds.  Derive one capture-scoped offset from the live tick
            # and normalize all bars and tick age to UTC consistently.
            broker_offset = _rounded_minute_offset(broker_time, capture)
            normalized_tick_time = broker_time - broker_offset
            frames = list(REQUIRED_TIMEFRAMES) + (["H4"] if include_h4 else [])
            timeframes = []
            for tf in frames:
                ...
                minutes = TIMEFRAME_MINUTES[tf]
                for row in rates:
                    open_time = datetime.fromtimestamp(int(row["time"]), tz=timezone.utc) - broker_offset
                    close_time = open_time + timedelta(minutes=minutes)
                    frame_bars.append(
                        {
                            "bar_id": _bar_id(symbol, tf, close_time),
                ...
                timeframes.append({"timeframe": tf, "requested_bars": bars, "returned_bars": len(frame_bars), "last_closed_bar_time": frame_bars[-1]["close_time"], "retrieved_at": iso_z(retrieved_at), "bars": frame_bars})
            positions = []
            ...
            snapshot = {
                ...
                "last_tick_time": iso_z(normalized_tick_time),
                "quote": {
