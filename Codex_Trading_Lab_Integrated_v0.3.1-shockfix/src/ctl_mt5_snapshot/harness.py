...
def _candidate_suppression_breakdown(decision: dict[str, Any]) -> dict[str, int]:
    ...
    data_quality = market.get("data_quality", {})
    if data_quality.get("freshness") == "STALE":
        counts["STALE_DATA_BLOCK"] += 1
    if data_quality.get("freshness") in {"BLOCKED", "INCOMPLETE", "MIXED_TIME"} or data_quality.get("qc_decision") != "PASS":
        counts["SNAPSHOT_QC_BLOCK"] += 1
    ...
    for scenario in scenario_packet["scenarios"]:
        if scenario["scenario_id"] in candidates_by_scenario:
            continue
        if scenario["status"] not in {"READY_FOR_ENTRY_EVALUATION", "CONFIRMING", "ARMED", "WATCHING"}:
...
def run_integration_harness(
    *,
    ...
    runtime = LiveRuntime(output_root / "runtime", symbol, require_live_source=False)
    ...
    snapshots_with_ready_candidate = 0
    ...
    for index in range(iterations):
        ...
        for candidate in candidate_items:
            ...
            if status == "READY_FOR_PERMISSION_REVIEW":
                ...
                snapshot_ready_candidates += 1
            elif status == "WAIT":
                wait_candidate_count += 1
        ...
        if snapshot_ready_candidates:
            snapshots_with_ready_candidate += 1
    ...
    return {
        ...
        "expired_candidate_count": expired_candidate_count,
        "snapshots_without_candidate": snapshots_without_candidate,
        "snapshots_with_candidate": snapshots_with_candidate,
        "snapshots_with_ready_candidate": snapshots_with_ready_candidate,
