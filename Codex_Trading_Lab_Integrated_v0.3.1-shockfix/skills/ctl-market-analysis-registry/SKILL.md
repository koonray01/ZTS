...
5. Freeze measurable Chat predictions through an available structured integration. If unavailable, report `CHAT_REGISTRATION_BLOCKED`; do not misrepresent launcher capabilities. Incomplete persisted provenance is `EXTERNAL_EVIDENCE_PARTIAL`.
6. Record to `D:\MyWork\AlgoTrade\OS\Zenith Trading System\runtime\analysis_registry`. Resolve its configured implementation; never fall back to a checkout-local Registry. Missing or ambiguous configuration blocks registration.
7. Run bounded foreground catch-up and report processed and remaining jobs. This is not a background service.
