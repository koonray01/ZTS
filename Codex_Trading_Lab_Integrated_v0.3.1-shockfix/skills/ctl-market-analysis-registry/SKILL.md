...
1. Invoke `D:\MyWork\AlgoTrade\OS\Zenith Trading System\tools\run_zenith_analysis.ps1` for a new current/live request. Add `-FourTierSetups` (Python: `--four-tier-setups`) for Scalping, Daytrade, both-horizon, or low-strictness setup requests. Never reuse an earlier snapshot as fresh.
...
## Four-tier setup contract

From one snapshot, build BUY and SELL rows for both `SCALPING` and `DAYTRADE` at `EXPLORATORY`, `VERY_RELAXED`, `RELAXED`, and `NORMAL` strictness. Freeze geometry and provenance before follow-up evidence. Keep each row in `WAITING_ACTIVATION` until its trigger occurs; expiry without entry is `EXPIRED_UNTRIGGERED`. Never backfill or tune entry, stop, target, activation, or expiry retrospectively.

Treat the four strictness rows for one prediction family, semantic opportunity, and generation as one headline opportunity. Preserve variant-level cohorts for comparison. This workflow uses one fresh snapshot and one Registry registration.
...
For four-tier output, also return `setup_generation_id`, `setup_variant_count`, `scorable_setup_count`, `non_scorable_setup_count`, and `scheduled_jobs`.
