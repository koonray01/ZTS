...
def _candidate_lifecycle_key(candidate: dict[str, Any]) -> str:
    """Stable comparison key; candidate_id deliberately remains snapshot-scoped."""
...
def diff_decision_state(
    ...
    *,
    ...
    def emit(event_type: str, entity_id: str, before: Any, after: Any, significant: bool = True) -> None:
        key = sanitize_id(f"{event_type}_{entity_id}_{after}")
        if key in seen:
            return
        seen.add(key)
        events.append(
    ...
    if previous is None:
        ...
        prev_entries = {
            _candidate_lifecycle_key(item): item
            for item in previous["entry_packet"]["candidates"]
        }
        for candidate in current["entry_packet"]["candidates"]:
            lifecycle_key = _candidate_lifecycle_key(candidate)
            before = prev_entries.get(lifecycle_key)
            if before and before["status"] != candidate["status"]:
                if candidate["status"] == "READY_FOR_PERMISSION_REVIEW":
                    emit("ENTRY_WINDOW_OPENED", lifecycle_key, before["status"], candidate["status"])
                elif candidate["status"] == "INVALIDATED":
                    emit("ENTRY_INVALIDATED", lifecycle_key, before["status"], candidate["status"])
                elif candidate["status"] == "EXPIRED":
                    emit("ENTRY_EXPIRED", lifecycle_key, before["status"], candidate["status"])
